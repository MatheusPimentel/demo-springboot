...
public class ProjetoController {
    ...
    public ResponseEntity<ProjetoResponseDTO> save(@Valid @RequestBody ProjetoRequestDTO projetoDto) {
        ProjetoResponseDTO savedProjeto = projetoService.saveProjeto(projetoDto);
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
                .buildAndExpand(savedProjeto.id()).toUri();
        return ResponseEntity.created(location).body(savedProjeto);
    }
